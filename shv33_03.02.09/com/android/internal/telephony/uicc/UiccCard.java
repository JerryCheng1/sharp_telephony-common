...
public class UiccCard {
    ...
    private void onIccSwap(boolean isAdded) {
        if (this.mContext.getResources().getBoolean(17956939)) {
    ...
    private void promptForRestart(boolean isAdded) {
        synchronized (this.mLock) {
            String title;
            String message;
            String dialogComponent = this.mContext.getResources().getString(17039426);
                ...
                public void onClick(DialogInterface dialog, int which) {
                    synchronized (UiccCard.this.mLock) {
                        if (which == -1) {
                            ...
                            ((PowerManager) UiccCard.this.mContext.getSystemService("power")).reboot("SIM2");
                        }
                    }
                }
            };
            ...
            AlertDialog dialog = new Builder(this.mContext).setTitle(title).setMessage(message).setPositiveButton(r.getString(17040401), listener).create();
    ...
    private void promptInstallCarrierApp(String pkgName) {
        ...
        Resources r = Resources.getSystem();
        String message = r.getString(17040402);
        AlertDialog dialog = new Builder(this.mContext).setMessage(message).setNegativeButton(r.getString(17040404), listener).setPositiveButton(r.getString(17040403), listener).create();
        dialog.getWindow().setType(TelephonyEventLog.TAG_IMS_CALL_RECEIVE);
        dialog.show();
    }
