...
public class AdnRecordLoader extends Handler {
    ...
    AdnRecordLoader(IccFileHandler fh, UiccCardApplication app) {
        this(fh);
    ...
    public void updateEF(AdnRecord adn, int ef, int extensionEF, int recordNumber, String pin2, int extRecNum, Message response) {
        this.mEf = ef;
        this.mExtensionEF = extensionEF;
        this.mRecordNumber = recordNumber;
        this.mUserResponse = response;
        this.mPin2 = pin2;
        if (extRecNum <= 0 || adn.mNumber.length() <= 20 || adn.mNumber.length() > 40) {
            this.mFh.getEFLinearRecordSize(ef, getEFPath(ef), obtainMessage(4, adn));
            return;
        }
        byte[] extData = adn.buildExtData();
        if (extData != null) {
            Rlog.d(LOG_TAG, "updateEf : Update the ext rec : " + extRecNum);
            this.mFh.updateEFLinearFixed(this.mExtensionEF, getEFPath(this.mExtensionEF), extRecNum, extData, this.mPin2, obtainMessage(6, extRecNum, 0, adn));
        }
    }
            ...
            switch (msg.what) {
                ...
                case 3:
                    ...
                    if (ar.exception == null) {
                        ...
                        for (int i = 0; i < s; i++) {
                            ...
                            this.mAdns.add(adn);
                            if (adn.hasExtendedRecord() && this.mExtensionEF != 0) {
                                this.mPendingExtLoads++;
                                this.mFh.loadEFLinearFixed(this.mExtensionEF, adn.mExtRecord, obtainMessage(2, adn));
                            }
                        }
                ...
                case 6:
                    ...
                    if (ar.exception == null) {
                        ...
                        this.mPendingExtLoads = 1;
                        this.mFh.getEFLinearRecordSize(this.mEf, getEFPath(this.mEf), obtainMessage(4, adn));
                        break;
                    }
                    adn.mExtRecord = 255;
                    throw new RuntimeException("update EF adn record failed", ar.exception);
