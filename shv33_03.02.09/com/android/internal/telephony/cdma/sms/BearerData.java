...
public final class BearerData {
    ...
    private static class Gsm7bitCodingResult {
        ...
        /* synthetic */ Gsm7bitCodingResult(Gsm7bitCodingResult gsm7bitCodingResult) {
            this();
        }
    ...
    private static void encodeUserDataPayload(UserData uData) throws CodingException {
        ...
        if (!uData.msgEncodingSet) {
            try {
                if (OUTPUT_DEBUG_LOG) {
                    Rlog.d(LOG_TAG, "encodeUserDataPayload calling encode7bitAscii(), set ENCODING_7BIT_ASCII");
                }
                uData.payload = encode7bitAscii(uData.payloadStr, false);
                uData.msgEncoding = 2;
            } catch (CodingException e) {
                if (OUTPUT_DEBUG_LOG) {
                    Rlog.d(LOG_TAG, "encodeUserDataPayload calling encodeUtf16(), set ENCODING_UNICODE_16");
                }
    ...
    private static String decodeUtf8(byte[] data, int offset, int numFields) throws CodingException {
        if (OUTPUT_DEBUG_LOG) {
            Rlog.d(LOG_TAG, "decodeUtf8 called.");
        }
    ...
    private static String decodeUtf16(byte[] data, int offset, int numFields) throws CodingException {
        if (OUTPUT_DEBUG_LOG) {
            Rlog.d(LOG_TAG, "decodeUtf16 called.");
        }
    ...
    private static String decode7bitAscii(byte[] data, int offset, int numFields) throws CodingException {
        if (OUTPUT_DEBUG_LOG) {
            Rlog.d(LOG_TAG, "decode7bitAscii called.");
        }
    ...
    private static String decode7bitGsm(byte[] data, int offset, int numFields) throws CodingException {
        if (OUTPUT_DEBUG_LOG) {
            Rlog.d(LOG_TAG, "decode7bitGsm called.");
        }
        int offsetBits = offset * 8;
        int offsetSeptets = (offsetBits + 6) / 7;
    ...
    private static String decodeShiftJis(byte[] data, int offset, int numFields) throws CodingException {
        if (OUTPUT_DEBUG_LOG) {
            Rlog.d(LOG_TAG, "decodeShiftJis called.");
        }
    ...
    private static void decodeUserDataPayload(UserData userData, boolean hasUserDataHeader) throws CodingException {
        ...
        switch (userData.msgEncoding) {
            case 0:
                boolean decodingtypeUTF8 = Resources.getSystem().getBoolean(17956967);
            ...
            case 4:
                ...
                if (OUTPUT_DEBUG_LOG) {
                    Rlog.d(LOG_TAG, "encoding is ENCODING_UNICODE_16");
                    return;
                }
                return;
            case 5:
            ...
            case 10:
                if (OUTPUT_DEBUG_LOG) {
                    Rlog.d(LOG_TAG, "ENCODING_GSM_DCS start");
                }
                if (hasUserDataHeader) {
                    if (OUTPUT_DEBUG_LOG) {
                        Rlog.d(LOG_TAG, "UserData has Header");
                    }
                    int dcs_copyLen = (userData.numFields < userData.payload.length ? userData.numFields : userData.payload.length) - ((userData.payload[0] & 255) + 1);
                    byte[] dcs_payload = new byte[dcs_copyLen];
                    System.arraycopy(userData.payload, offset, dcs_payload, 0, dcs_copyLen);
                    userData.payload = dcs_payload;
                    userData.payloadStr = decodeUtf16(userData.payload, 0, (userData.numFields - offset) / 2);
                } else {
                    userData.payloadStr = decodeGsmDcs(userData.payload, offset, userData.numFields, userData.msgType);
                }
                if (OUTPUT_DEBUG_LOG) {
                    Rlog.d(LOG_TAG, "decoded userData = '" + userData.toString() + "'");
                    Rlog.d(LOG_TAG, "ENCODING_GSM_DCS End");
                    return;
                }
                return;
            default:
