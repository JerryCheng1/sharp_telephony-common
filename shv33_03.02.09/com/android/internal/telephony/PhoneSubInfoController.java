...
public class PhoneSubInfoController extends Stub {
    ...
    public String getMccMncOnSimLock() {
        return getMccMncOnSimLock(getDefaultSubscription());
    }

    public String getMccMncOnSimLock(int subId) {
        Phone phone = getPhone(subId);
        if (phone != null) {
            return phone.getMccMncOnSimLock();
        }
        loge("getMccMncOnSimLock phone is null for Subscription:" + subId);
        return null;
    }
    ...
    public int getBrand() {
        return getBrand(getDefaultSubscription());
    }

    public int getBrand(int subId) {
        Phone phone = getPhone(subId);
        if (phone != null) {
            return phone.getBrand();
        }
        loge("getBrand phone is null for Subscription:" + subId);
        return 0;
    }
