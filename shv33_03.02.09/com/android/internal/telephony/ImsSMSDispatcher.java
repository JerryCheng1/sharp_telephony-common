...
public class ImsSMSDispatcher extends SMSDispatcher {
    ...
    /* Access modifiers changed, original: protected */
    public SmsTracker getNewSubmitPduTracker(String destinationAddress, String scAddress, String message, SmsHeader smsHeader, int format, PendingIntent sentIntent, PendingIntent deliveryIntent, boolean lastPart, int priority, boolean isExpectMore, int validityPeriod, AtomicInteger unsentPartCount, AtomicBoolean anyPartFailed, Uri messageUri, String fullMessageText) {
        Rlog.e(TAG, "Error! Not implemented for IMS.");
        return null;
    }
    ...
    public PendingIntent checkDeliveryPendingList(com.android.internal.telephony.cdma.SmsMessage sms) {
        return this.mGsmDispatcher.isUpdateDeliveryPendingList(sms.mMessageRef);
    }

    public PendingIntent checkDeliveryPendingList(com.android.internal.telephony.gsm.SmsMessage sms) {
        return this.mCdmaDispatcher.isUpdateDeliveryPendingList(sms.mMessageRef);
    }

    /* Access modifiers changed, original: protected */
    public PendingIntent isUpdateDeliveryPendingList(int messageRef) {
        Rlog.d(TAG, "isUpdateDeliveryPendingList should never be called from here!");
        return null;
    }
    ...
    public boolean shouldSendSmsOverIms() {
        boolean sendSmsOn1x = this.mContext.getResources().getBoolean(17956986);
        State currentCallState = this.mPhone.getState();
        int currentVoiceNetwork = this.mPhone.getServiceState().getVoiceNetworkType();
        ...
        if (!sendSmsOn1x || currentDataNetwork != 14 || currentVoiceNetwork != 7 || currentCallState == State.IDLE) {
            return true;
        }
