...
public enum DcFailCause {
    ...
    PROTOCOL_ERRORS(CallFailCause.PROTOCOL_ERROR_UNSPECIFIED),
    APN_TYPE_CONFLICT(QtiCallConstants.CALL_FAIL_EXTRA_CODE_LOCAL_LOW_BATTERY),
    ...
    private DcFailCause(int errorCode) {
        this.mRestartRadioOnRegularDeactivation = Resources.getSystem().getBoolean(17957019);
    ...
    public boolean isPermanentFail(Context context, int subId) {
        ...
        if (this == ACTIVATION_REJECT_GGSN) {
            return getConfigItem(context, subId, "reject_ggsn_perm_failure");
        }
        if (this == PROTOCOL_ERRORS) {
            return getConfigItem(context, subId, "protocol_errors_perm_failure");
        }
