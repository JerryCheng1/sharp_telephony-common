...
public class UsimPhoneBookManager extends Handler implements IccConstants {
            ...
            while (i < numRecs) {
                try {
                    ...
                    if (!(email == null || email.equals(""))) {
                        ...
                        int index = ((CallFailCause.ERROR_UNSPECIFIED & adnEfid) << 8) | ((adnRecId - 1) & 255);
    ...
    private boolean buildType2EmailList(int recId) {
        ...
        for (int i = 0; i < numRecs; i++) {
            try {
                ...
                if (!(email == null || email.equals(""))) {
                    int index = ((CallFailCause.ERROR_UNSPECIFIED & adnEfid) << 8) | (i & 255);
                    ArrayList<String> emailList = (ArrayList) this.mEmailsForAdnRec.get(index);
                    if (emailList == null) {
                        emailList = new ArrayList();
                    }
    ...
    private void updatePhoneAdnRecord() {
        ...
        for (int i = 0; i < numAdnRecs; i++) {
            ...
            try {
                ArrayList<String> emailList = (ArrayList) this.mEmailsForAdnRec.get(((CallFailCause.ERROR_UNSPECIFIED & rec.getEfid()) << 8) | ((rec.getRecId() - 1) & 255));
                if (emailList != null) {
