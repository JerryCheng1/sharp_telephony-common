...
public class IccUtils {
    ...
    static byte[] stringToAdnStringField(String str) {
        boolean z = false;
        int i = 0;
        while (i < str.length()) {
            try {
                GsmAlphabet.countGsmSeptets(str.charAt(i), true);
                i++;
            } catch (EncodeException e) {
                z = true;
            }
        }
        return stringToAdnStringField(str, z);
    }

    static byte[] stringToAdnStringField(String str, boolean z) {
        if (!z) {
